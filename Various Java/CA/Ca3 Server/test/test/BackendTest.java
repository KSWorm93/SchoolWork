...
public class BackendTest {
    ...
    public void searchCompanyByName() {
        ...
        given().
                contentType("application/json").
                header("Authorization", "Bearer " + from(json).get("token")).
                body("{'option':'name', 'searchText':'Dong', 'ctry':'dk'}").
                when().
                post("/cvrSearch/getcomp/option/searchText/ctry").
                then().
                statusCode(200);
    }

    @Test
    public void failSearchCompanyByName() {
        //First, make a login to get the token for the Authorization, saving the response body in String json
        String json = given().
                contentType("application/json").
                body("{'username':'user','password':'test'}").
        ...
        given().
                ...
                header("Authorization", "Bearer " + from(json).get("token")).
                when().
                post("/cvrSearch/getcomp/name/tesadwnjwadn/dk").
                then().
                statusCode(404);
    }

    @Test
    public void valutaConvertEnterString() {
